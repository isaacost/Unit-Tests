...
function addItem(restaurante, novoItem) {
  restaurante.consumption.push(novoItem);
}
...
const createMenu = (menu) => ({
    ...
    consumption: [],
    order(novoItem) {
      addItem(this, novoItem);
    },
    pay() {
      let conta = 0;
      const drinks = this.fetchMenu().drink;
      const foods = this.fetchMenu().food;
      for (const novoItem of this.consumption) {
        if (drinks[novoItem]) {
          conta += drinks[novoItem];
        }
         if (foods[novoItem]) {
          conta += foods[novoItem];
          }
      }
      let gorjeta = conta / 10;
      conta += gorjeta;
      return conta;
    },
